...
experiment_dir="${EXPERIMENT_DIR:-outputs/spider_iterative_cluster_mab}"
result_dir="${RESULT_DIR:-results/spider_iterative_cluster_mab}"
lock_dir=".cache/iterative_cluster_mab.lock"
pid_file=".cache/iterative_cluster_mab.pid"
...
if ! mkdir "$lock_dir" 2>/dev/null; then
  echo "Another cluster-MAB pipeline appears to be active: $lock_dir" >&2
  exit 1
fi
echo "$$" > "$pid_file"
...
echo "Started five-round IDU plus cluster-MAB ablation: $(date)"
echo "Seeds: $seeds"
...
for seed in $seeds; do
  ...
  "$python_bin" src/experiments/run_iterative_lead_spider.py \
    ...
    --smoothing 0.1 --gamma 0.06 --selection-policy exp3_cluster_global
  "$python_bin" src/analysis/validate_iterative_run.py --run-dir "$run_dir"
done

read -r -a seed_array <<< "$seeds"
"$python_bin" src/analysis/summarize_iterative_cluster_mab_ablation.py \
...
echo "Completed five-round IDU plus cluster-MAB ablation: $(date)"
