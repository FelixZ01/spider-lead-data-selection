...
experiment_dir="${EXPERIMENT_DIR:-outputs/spider_static_uncertainty_multiround}"
result_dir="${RESULT_DIR:-results/spider_static_uncertainty_multiround}"
lock_dir=".cache/static_uncertainty_multiround.lock"
pid_file=".cache/static_uncertainty_multiround.pid"
...
if ! mkdir "$lock_dir" 2>/dev/null; then
  echo "Another static-uncertainty pipeline appears to be active: $lock_dir" >&2
  exit 1
fi
echo "$$" > "$pid_file"
...
echo "Started five-round static-uncertainty control: $(date)"
echo "Seeds: $seeds"
...
for seed in $seeds; do
  ...
  "$python_bin" src/experiments/run_iterative_lead_spider.py \
    ...
    --smoothing 0.1 --gamma 0.06 --selection-policy static_uncertainty
  "$python_bin" src/analysis/validate_iterative_run.py --run-dir "$run_dir"
done

read -r -a seed_array <<< "$seeds"
"$python_bin" src/analysis/summarize_static_uncertainty_multiround.py \
...
echo "Completed five-round static-uncertainty control: $(date)"
