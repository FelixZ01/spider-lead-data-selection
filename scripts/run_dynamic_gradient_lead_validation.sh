...
seed="${SEED:-42}"
output_dir="${OUTPUT_DIR:-outputs/spider_dynamic_gradient_lead/seed_$seed}"
...
"$python_bin" src/experiments/run_iterative_lead_spider.py \
  ...
  --training-mode new_batch --total-training-steps 1500 \
  --epochs-per-round 3 --eval-samples 1034 --seed "$seed" \
  --smoothing 0.1 --gamma 0.06 \
  --selection-policy gradient_lead --max-reuse 4 --device auto
