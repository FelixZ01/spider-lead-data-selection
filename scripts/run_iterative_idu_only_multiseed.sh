...
experiment_dir="${EXPERIMENT_DIR:-outputs/spider_iterative_idu_only}"
result_dir="${RESULT_DIR:-results/spider_iterative_idu_only}"
lock_dir=".cache/iterative_idu_only_multiseed.lock"
pid_file=".cache/iterative_idu_only_multiseed.pid"
...
if ! mkdir "$lock_dir" 2>/dev/null; then
  echo "Another IDU-only pipeline appears to be active: $lock_dir" >&2
  exit 1
fi
echo "$$" > "$pid_file"
...
echo "Started five-round IDU-only ablation: $(date)"
echo "Seeds: $seeds"
...
for seed in $seeds; do
  ...
  "$python_bin" src/experiments/run_iterative_lead_spider.py \
    ...
    --smoothing 0.1 --gamma 0.06 --selection-policy global_idu
  "$python_bin" src/analysis/validate_iterative_run.py --run-dir "$run_dir"
done

read -r -a seed_array <<< "$seeds"
"$python_bin" src/analysis/summarize_iterative_idu_ablation.py \
  --experiment-dir "$experiment_dir" \
  --required-metrics results/spider_required_q1_q3/metrics.json \
  --five-round-metrics results/spider_iterative_multiround_cumulative/metrics.json \
...
echo "Completed five-round IDU-only ablation: $(date)"
