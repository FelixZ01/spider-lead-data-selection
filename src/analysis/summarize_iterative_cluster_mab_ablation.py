#!/usr/bin/env python3
"""Summarise the IDU plus clustering and EXP3 component ablation."""
...
def main() -> None:
    ...
    full_adaptation = read_json(args.full_adaptation_metrics)
    ...
    for seed in args.seeds:
        ...
        if summary.get("selection_policy") != "exp3_cluster_global":
            raise ValueError(f"seed {seed} is not a cluster-MAB ablation")
        ...
        full_adaptation_score = float(
            full_adaptation_by_seed[seed]["official_spider_exact_match"]
        )
        ...
        runs.append({
            ...
            "full_adaptation_exact_match": full_adaptation_score,
            "random_exact_match": random_score,
            "full_data_exact_match": full_score,
            "difference_vs_idu_only": score - idu_score,
            "difference_vs_full_adaptation": score - full_adaptation_score,
    ...
    result = {
        "question": (
            "Do difficulty clustering and EXP3 allocation improve the IDU-only "
            "method when database-group allocation is removed?"
        ),
        "controlled_setup": {
        ...
        "wins_vs_idu_only": sum(
            row["difference_vs_idu_only"] > 0 for row in runs
        ),
        ...
        "method_boundary": (
            "Five-round observed-loss-change utility with two difficulty clusters "
            "and EXP3 allocation. Within the chosen cluster, samples are selected "
            "globally by utility; database-group quotas are removed."
        ),
    }
