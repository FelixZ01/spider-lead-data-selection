#!/usr/bin/env python3
"""Score every input row with target-token cross entropy from a CodeT5 checkpoint."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--score-key", default="current_target_loss")
...
def main() -> None:
    ...
    for offset in range(0, len(rows), args.batch_size):
        ...
        losses = token_losses.sum(dim=1) / valid_tokens
        for row, loss in zip(batch_rows, losses.detach().cpu().tolist()):
            row[args.score_key] = float(loss)
        completed = min(offset + args.batch_size, len(rows))
        if completed % 100 == 0 or completed == len(rows):
            print(f"Scored {completed}/{len(rows)} with {args.model}")
